  ...
  helpers do
    def message_params
      ...
      if params[:message][:object_id]
        product_id = Product.decrypt(Product.encrypted_id_key, params[:message][:object_id])
        params[:message][:object_id] = product_id
      end
      # Decrypt parent id
      if params[:message][:parent_id]
        parent_id = Message.decrypt(Message.encrypted_id_key, params[:message][:parent_id])
