  ...
  def self.confirm_msg(order, type)
    ...
    when 0
      if order.payment_total != 0
        "您的物品加运费优惠后共#{order.total}元，您已经支付成功#{order.payment_total}元。初七依次发货，保密包装请耐心等待。【首趣商城】"
    ...
    when 1
      ...
      if order.shipping_address.blank? && order.name.blank?
        "过年好！#{prepend_text}物品加运费共#{order.total}元，保密包装货到付款。回复您的具体地址(省市区县街道)和姓名初七发货。【首趣商城】"
        # "您好，#{prepend_text}物品加运费共#{order.total}元，保密包装。回复您的具体地址省市区县街道和姓名 立刻发货，电话：4007065868【首趣商城】"
      elsif order.shipping_address.blank?
        "过年好！#{prepend_text}物品加运费共#{order.total}元，保密包装货到付款。回复您的具体地址(省市区县街道)初七发货。【首趣商城】"
        # "您好，#{prepend_text}物品加运费共#{order.total}元，保密包装。回复您的具体地址(省市区县街道) 立刻发货，电话：4007065868【首趣商城】"
      else
        "过年好！#{prepend_text}物品加运费共#{order.total}元，保密包装货到付款。回复数字1确认初七发货。【首趣商城】"
